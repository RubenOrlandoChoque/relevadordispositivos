 ...
 */
public class FotosRuteosSyncTask extends AsyncTask<String, Integer, Boolean> {
    ProgressDialog loading = null;
    private Context activity;

    public FotosRuteosSyncTask(Context _activity) {
        activity = _activity;
    }
    ...
    protected Boolean doInBackground(String... params) {
        ...
        final String METHOD_NAME = "SyncRuteoFiles";
        final String SOAP_ACTION = NAMESPACE + METHOD_NAME;

        boolean resul = true;
        String codFileTmp = "";
        Cursor cursor = null;
        cursor = Data.getConn().executeSelect(Querys.getRuteosFotos(), cursor);
        while (cursor.moveToNext()) {
            SoapObject request = new SoapObject(NAMESPACE, METHOD_NAME);
            request.addProperty("nameFile",cursor.getString(10));
            codFileTmp = cursor.getString(1);

            File file = new File(Environment.getExternalStorageDirectory()+"/"+Data.DIRECTORIO_FOTO +"/"+ cursor.getString(10));
            ...
            try {
                ...
                if (res.equals("true")) {
                    ...
                    ruteo.getRuteoByCodRuteo(cursor.getString(1));
    ...
    protected void onPreExecute() {
        ...
        loading.setTitle("Sincronizando fotos de ruteos");
        ...
        loading.setButton(DialogInterface.BUTTON_NEGATIVE, "Cancelar", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                FotosRuteosSyncTask.this.cancel(true);
                Data.setSyncAll(false);
                Intent intent  =new Intent("FOTOS_RUTEOS_SYNC_FINISH");
    ...
    protected void onPostExecute(Boolean result) {
        ...
        if(Data.isSyncAll()){
            //new TrackingSyncTask(activity).execute();
            TrackingSyncTask asyncTask =new TrackingSyncTask(activity);
